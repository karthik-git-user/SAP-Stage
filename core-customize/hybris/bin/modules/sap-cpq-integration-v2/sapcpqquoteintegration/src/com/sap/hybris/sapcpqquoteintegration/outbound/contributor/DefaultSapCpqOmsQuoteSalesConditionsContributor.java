 ...
 */
package com.sap.hybris.sapcpqquoteintegration.outbound.contributor;
...
public class DefaultSapCpqOmsQuoteSalesConditionsContributor extends SapOmsSalesConditionsContributor {



	private static final Logger LOG = Logger.getLogger(DefaultSapCpqOmsQuoteSalesConditionsContributor.class);

	@Override
	protected void createOrderDiscountRows(final OrderModel order, final List<Map<String, Object>> result)
	{
		LOG.info("DefaultSapCpqOmsQuoteSalesConditionsContributor : OrderId = " + order.getCode());
		...
		for (final DiscountValue discountValue : emptyIfNull(discounts))
		{
			...
			else
			{
				...
				{
					final String conditionCode = order.getStore().getSAPConfiguration().getCpqQuoteDiscountConditionCode();
                       ...
                       if (disVal.getCode().startsWith(PROMOTION_DISCOUNT_CODE_PREFIX)) {
                        ...
                        // Add other kinds of discounts
                           if (disVal.getCode().equals("QuoteEntryDiscount"))
                           {
                                   ...
                                   final String conditionCode = order.getStore().getSAPConfiguration().getCpqQuoteEntryDiscountConditionCode();
                                   row.put(SalesConditionCsvColumns.CONDITION_CODE, conditionCode);
                           }
                           else
                           {
