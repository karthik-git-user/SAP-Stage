...
class ItemKeyDescriptorUnitTest extends Specification {
    ...
    @Test
    def "throws MissingKeyReferencedAttributeValueException when reference key attribute is null"() {
        given:
        def companyIO = item 'Company', [uniqueAttribute('name')]
        def positionIO = item 'Position', [uniqueAttribute('code')]
        def company = 'company'
        def integrationObjectItem = item([
                uniqueAttribute(company, companyIO),
                uniqueAttribute('position', positionIO),
                nonUniqueAttribute('description')])
        def descriptor = ItemKeyDescriptor.create(integrationObjectItem)
        def dataItem = [description: 'Good Job', (company): null, position: [code: 'dev123']]

        when:
        descriptor.calculateKey(dataItem)

        then:
        def e = thrown(MissingKeyReferencedAttributeValueException)
        e.message == "Key referenced attribute [$company] is required for [$ITEM_CODE]."
    }
