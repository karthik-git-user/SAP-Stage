...
class ODataNullableIntegrationKeyPropertiesPersistenceFacadeIntegrationTest extends ServicelayerTransactionalSpockSpecification {
    ...
    @Test
    def "cannot persist without providing nullable integration key referenced attributes"() {
        given:
        importImpEx(
                'INSERT_UPDATE IntegrationObjectItem; integrationObject(code)[unique = true]; code[unique = true]; type(code)',
                '; OrderEntryExample ; Address        ; Address',

                'INSERT_UPDATE IntegrationObjectItemAttribute; integrationObjectItem(integrationObject(code), code)[unique = true]; attributeName[unique = true]; attributeDescriptor(enclosingType(code), qualifier); returnIntegrationObjectItem(integrationObject(code), code); unique[default = false]; autoCreate[default = false]',
                '; OrderEntryExample:Order          ; deliveryAddress   ; Order:deliveryAddress        ; OrderEntryExample:Address ; true ; true',

                '; OrderEntryExample:Address        ; firstname         ; Address:firstname            ;                           ; true')

        def body = orderEntryWithoutAddressJson()
        def postContext = ODataFacadeTestUtils.createContext(
                oDataPostRequest(SERVICE_NAME, "OrderEntries", body, Locale.ENGLISH, MediaType.APPLICATION_JSON_VALUE))
        when:
        def postResponse = facade.handleRequest(postContext)
        ...
        then:
        postResponse.getStatus() == HttpStatusCodes.BAD_REQUEST
        and:
        def json = JsonObject.createFrom postResponse.entityAsStream
        json.getString('error.code') == 'missing_key'
        and:
        json.getString('error.message.value') == "Key NavigationProperty [deliveryAddress] is required for EntityType [Order]."
    }
    ...
    def orderEntryWithoutAddressJson() {
        JsonBuilder.json()
                .withField("order", orderWithOutAddressJson())
                .withField("product", productJson())
                .withField("unit", unitJson())
                .withField("quantity", "25")
                .withField("entryNumber", 321).build()
    }

    def addressJson() {
        JsonBuilder.json()
                .withField("firstname", "Jade").build()
    }
