...
class HttpServletRequestToODataRequestConverterUnitTest extends Specification {
    ...
    def 'converts one arbitrary header'() {
        ...
        then:
        convertedReq.requestHeaders == [integrationuser:[null], someheader: ['header-value']]
    }
    ...
    def 'converts multiple headers with different value types into headers with String values'() {
        ...
        then:
        convertedReq.requestHeaders == ['content-type': [APPLICATION_JSON], 'header-one': ['1'], 'header-two': ['true'], integrationuser: [null]]
    }

    @Test
    @Unroll
    def 'converted headers contain user #user only even when original request does not have headers'() {
        given:
        def request = createHttpServletRequest(HttpMethod.GET.name())
        request.setRemoteUser(user)

        when:
        def convertedReq = converter.convert request

        then:
        convertedReq.requestHeaders == [integrationuser: [user]]
