...
class ODataFacadeInboundMonitoringIntegrationTest extends ServicelayerTransactionalSpockSpecification {
	...
	@Test
	@Unroll
	def "inbound request has #method method when #method request is made"() {
		...
		where:
		method   | request
		'PATCH'  | patch('Staged|Default|abc', product().withCode('abc').withName('new'))
		'DELETE' | delete('Staged|Default|abc')
		'POST'   | post(product())
	}
	...
	def batchODataPostRequest(String content) {
		...
		request.getRequestHeaders().put Odata2servicesConstants.USER, [USER_ID]
	...
	ODataContext patch(String key, ODataJsonProductBuilder body) {
		ODataFacadeTestUtils.createContext ODataRequestBuilder.oDataPatchRequest()
				...
				.withHeader(Odata2servicesConstants.USER, USER_ID)
				.withBody(body.build())
	}
